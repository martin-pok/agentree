...
async function makeRepo() {
  const dir = await tempDir('agentree-git-');
  g(dir, 'init', '-q', '-b', 'main');
...
test('git: pomocné funkce — bezpečné větve, slug, adresa bez hesla, status', () => {
  for (const ok of ['main', 'agentree/claude-cenik-1430', 'feature/x.y']) assert.equal(isSafeRef(ok), true, ok);
...
test('git: pracovní kopie agenta → změny → přijetí (commit + sloučení) → úklid', async () => {
  const repo = await makeRepo();
  const wtRoot = await tempDir('agentree-wt-');
  const dir = path.join(wtRoot, 'claude-cenik');
  assert.equal((await createWorktree({ repo, base: 'main', branch: '-bad', dir })).ok, false);
  assert.equal((await createWorktree({ repo, base: 'neni', branch: 'agentree/x', dir })).error, 'Větev neni v repozitáři neexistuje.');
  const wt = await createWorktree({ repo, base: 'main', branch: 'agentree/claude-cenik', dir });
  assert.equal(wt.ok, true, wt.error);
  assert.equal((await createWorktree({ repo, base: 'main', branch: 'agentree/jina', dir })).ok, false, 'stejná složka podruhé ne');
  ...
  const accepted = await acceptWork({ repo, dir, branch: 'agentree/claude-cenik', base: 'main', message: 'Agentree: Claude Code — ceník' });
  assert.deepEqual(accepted, { ok: true, merged: true });
  assert.equal(await fs.readFile(path.join(repo, 'cenik.html'), 'utf8'), '<h1>Ceník</h1>\n');
  assert.match(g(repo, 'log', '--oneline', '-3'), /Agentree: Claude Code — ceník/);

  const cleaned = await cleanupWork({ repo, dir, branch: 'agentree/claude-cenik' });
...
test('git: konflikt se bezpečně vrátí, neuložené změny v hlavní složce blokují, zahození odstraní vše', async () => {
  ...
  await createWorktree({ repo, base: 'main', branch: 'agentree/codex-readme', dir });
  await fs.writeFile(path.join(dir, 'README.md'), '# Web od Codexu\n');
  await fs.writeFile(path.join(repo, 'README.md'), '# Web ručně\n');

  const blocked = await acceptWork({ repo, dir, branch: 'agentree/codex-readme', base: 'main' });
  assert.equal(blocked.ok, false);
  assert.match(blocked.error, /neuložené změny/);

  g(repo, 'commit', '-q', '-am', 'Ruční úprava');
  const head = g(repo, 'rev-parse', 'HEAD');
  const conflict = await acceptWork({ repo, dir, branch: 'agentree/codex-readme', base: 'main' });
  ...
  assert.match((await acceptWork({ repo, dir, branch: 'agentree/codex-readme', base: 'main' })).error, /je na větvi jina/);
  g(repo, 'checkout', '-q', 'main');

  assert.deepEqual(await discardWork({ repo, dir, branch: 'agentree/codex-readme' }), { ok: true });
