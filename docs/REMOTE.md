...
Agentree z principu poslouchá jen v domácí síti (`docs/SECURITY.md#přístup-z-telefonu`). To
...
Řešení není vlastní server Agentree v cloudu. Je to **tunel, který si spustíš sám** —
Agentree jen zjistí, jestli ho máš, a poradí, který z nich použít (`src/tunnel.js`).
...
Agentree tyto tři cesty jen **detekuje** (`detectTunnels()`) a **doporučí** (`remoteAdvice()`) —
...
## Proč Agentree nemá vlastní server v cloudu

Agentree čte přepisy práce s AI agenty — kód, klientská data, prompty (`docs/SECURITY.md`).
...
žádná analytika, žádný účet u Agentree. Tunel podle této stránky nic nemění — pořád jde jen o
...
a service worker na straně Agentree — to je samostatná položka mimo tento modul.
...
Proto (a jen proto) v Agentree **zůstává povinné párování PINem a token** i po zapnutí
veřejného tunelu:
...
- token zůstává v `HttpOnly` cookie a v datech Agentree je jen jeho hash;
- ochrana proti CSRF (`X-Agentree` + kontrola `Origin`) platí bez ohledu na to, jestli je
