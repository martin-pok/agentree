...
const FILTERS = [
  ['all', 'Vše', () => true],
  ['unread', 'Nepřečtené', (a) => !a.read],
  ['needs_input', 'Rozhodnutí', (a) => a.kind === 'needs_input'],
  ['limit', 'Limity', (a) => a.kind === 'limit' || a.kind === 'limit_near'],
  ['budget', 'Rozpočet', (a) => a.kind === 'budget'],
  ['done', 'Dokončené', (a) => a.kind === 'done'],
];
...
function dayLabel(ts) {
  ...
  if (d === startOfDay(today - DAY)) return 'Včera';
  return dateLong(ts);
}
...
function mount(el) {
  ...
  el.innerHTML = `
    <div class="toolbar" data-enter style="--i:1">
      <div class="seg" role="group" aria-label="Filtrovat upozornění" data-region="filters"></div>
      <div class="toolbar-actions">
        <a class="btn" href="#/nastaveni">${ICON.sliders}Nastavit upozornění</a>
        <button class="btn" type="button" data-read-all-view>${ICON.check}Označit vše jako přečtené</button>
      </div>
    </div>
...
function update() {
  ...
  if (!list.length) {
    fill(el, 'list', emptyState({
      title: v.filter === 'all' ? 'Zatím žádná upozornění' : 'Nic k zobrazení',
      text: 'Upozorníme tě, když agent bude potřebovat rozhodnutí, narazí na limit, dokončí dlouhou úlohu nebo když překročíš rozpočet.',
    }));
    return;
  }
  let day = '';
  fill(el, 'list', `<ul class="alert-list">${list.map((a) => {
    ...
    return `${head}<li class="alert-item level-${esc(a.level)}${a.read ? '' : ' is-unread'}">
      ...
      <div class="alert-main">
        ...
        ${a.sessionId ? `<a class="link-inline" href="${agentHref(a.sessionId)}" data-read="${esc(a.id)}">Otevřít agenta ${ICON.arrow}</a>` : ''}
      </div>
      ${a.read ? '' : `<button class="icon-btn" type="button" data-read="${esc(a.id)}" aria-label="Označit jako přečtené">${ICON.check}</button>`}
    </li>`;
  }).join('')}</ul>`);
}

export default { id: 'upozorneni', title: 'Upozornění', mount, update, unmount: () => { v.el = null; } };
